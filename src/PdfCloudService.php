...
/**
 * [EN]    PAdES (PDF) signing — Cloud HSM / PSC (one-step).
 *         Uses cloud credentials (credentialId + SAD) instead of a local certificate.
 * [PT-BR] Assinatura PAdES (PDF) — Cloud HSM / PSC (passo único).
 *         Usa credenciais de nuvem (credentialId + SAD) em vez de certificado local.
 */
...
class PdfCloudService
{
    ...
    public function signBatch(string $inputPath, string $outputPath): ?string
    {
        ...
        $response = $this->client->post("$baseUrl/solidsign/dsig/pdf/sign-hsm-cloud", [
        ...
        return $this->downloadAndZip($signResp, $origNames, $auth, $outputPath, 'signed_pdf_cloud');
    }
    ...
    private function downloadAndZip(array $signResp, array $origNames, string $auth, string $outputDir, string $prefix): ?string
    {
        ...
        echo "PAdES Cloud signing complete. Output: $zipPath\n";
        return $zipPath;
    }
}
